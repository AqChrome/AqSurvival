/*
  Открыть страницу "Задачи": скрыть главное меню и показать список задач.
*/
...
// Если список уже существует — удалить и создать заново
private _old = uiNamespace getVariable ["AQPH_TasksGroup", controlNull];
...
_list ctrlCommit 0;
uiNamespace setVariable ["AQPH_TasksGroup", _list];
...
if (_tasks isEqualTo []) then {
...
} else {
  {
    ...
    _btn ctrlCommit 0;
    _btn ctrlAddEventHandler ["ButtonClick", format ["[%1] call AQPH_fnc_taskDetail;", _id]];
  } forEach _tasks;
};
...
_back ctrlCommit 0;
_back ctrlAddEventHandler ["ButtonClick", { [] call AQPH_fnc_tasksClose }];
