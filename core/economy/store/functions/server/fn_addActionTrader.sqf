/*
	Function: AQ_store_fnc_addActionTrader
	Author: HallyG
	...
	Example:
	[trader1, west] call AQ_store_fnc_addActionTrader;
...
if (isNil {_trader getVariable "AQ_store_trader_type"}) exitWith {};
 ...
 if (_target isEqualType false) then {
	if (hasInterface) then {
		[
			_trader,
			format [localize "STR_AQ_STORE_OPEN_ACTION", _trader getVariable ["AQ_store_name", localize "STR_AQ_STORE_ACTION"]],
			...
			{},
			{
				...
				[_trader] call AQ_store_fnc_openStore;
			}, {}, [], 0.1, nil, false, false
		] call BIS_fnc_holdActionAdd;
	};
} else {
	if (isServer) then {
		[_trader] remoteExecCall ["AQ_store_fnc_addActionTrader", _target, _trader];
	};
};
