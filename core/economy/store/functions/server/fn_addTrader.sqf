/*
	Function: AQ_store_fnc_addTrader
	Author: HallyG
	...
	Example:
	[obj2, "Navigation"] call AQ_store_fnc_addTrader;
...
try {
	if (!isNil {_trader getVariable "AQ_store_trader_type"}) then {throw ["Trader already initialised", __LINE__]};
	...
	if (!isClass (missionConfigFile >> "cfgAQAddons" >> "cfgAQStore" >> "stores" >> _traderType)) then {throw ["Invalid Trader type", __LINE__]};
	...
	private _categories = [
		getArray (missionConfigFile >> "cfgAQAddons" >> "cfgAQStore" >> "stores" >> _traderType >> "categories"),
		{getText (missionConfigFile >> "cfgAQAddons" >> "cfgAQStore" >> "categories" >> _x >> "displayname")},
		true
	] call AQ_fnc_sortArray;

	private _classes = [];
	private _stocks = [];
	{
		private _category = _x;
		_configCategory = missionConfigFile >> "cfgAQAddons" >> "cfgAQStore" >> "categories" >> _category;
	...
	_trader setVariable ["AQ_store_trader_type", _traderType, true];
	_trader setVariable ["AQ_store_trader_stocks", _stocks, true];
	...
	_trader setVariable ["AQ_store_name", getText (missionConfigFile >> "cfgAQAddons" >> "cfgAQStore" >> "stores" >> _traderType >> "displayName"), true];
	[_trader, _target] call AQ_store_fnc_addActionTrader;
	true
} catch {
	[_exception] call AQ_fnc_log;
