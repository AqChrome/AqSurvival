/*
	Function: AQ_store_fnc_getItemStats
	Author: HallyG, BIS
	...
	Example:
	["acc_flashlight"] call AQ_store_fnc_getItemStats;
__________________________________________________________________*/
params [
	["_config", configNull, [configNull]]
];

if (isNil "AQ_store_stats_weapons") exitWith {
	// Taken from BIS_fnc_arsenal, modified by HallyG
	AQ_store_stats_weapons = [
	...
	AQ_store_stats_equipment = [_statsEquipmentMin, _statsEquipmentMax];
	...
	AQ_store_stats_optics = [selectMin _statsOpticsMin, selectMax _statsOpticsMax];
};

call {
	if (getNumber (_config >> "type") in [1, 2, 4] && {getNumber (_config >> "isbackpack") != 1}) exitWith {
		AQ_store_stats_weapons params ["_statsMin", "_statsMax"];
	...
	if (getNumber (_config >> "itemInfo" >> "type") in [605,701,801] || (getNumber (_config >> "type") isEqualTo 1 && getNumber (_config >> "isbackpack") isEqualTo 1)) exitWith {
		AQ_store_stats_equipment params ["_statsMin", "_statsMax"];
	...
	if (isClass (_config >> 'ItemInfo' >> 'OpticsModes')) exitWith {
		...
		private _statZoom = linearConversion [AQ_store_stats_optics select 0, AQ_store_stats_optics select 1, _zoom, 0.01, 1, true];
