/*
	Function: AQ_store_fnc_canEquipItem
	...
	Example:
	[player, "acc_flashlight"] call AQ_store_fnc_canEquipItem;
	...
	case "Magazine": {
		{
			if (count (_x select 0) > 0) then {
				if ((_x select 1) isEqualTo []) then {
					_compatibleWeaponItems = ((_x select 0) call AQ_store_fnc_getCompatibleItems) apply {toLower _x};
