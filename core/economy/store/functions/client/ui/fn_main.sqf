/*
	...
	Example:
	["init"] call AQ_store_fnc_main;
__________________________________________________________________*/
#include "..\..\..\dialog\idcs.hpp"
#define CFG_STORE missionConfigFile >> "cfgAQAddons" >> "cfgAQStore";
...
private _trader = player getVariable ["AQ_store_trader_current", objNull];
...
switch (_mode) do {
	case ("onLoad"): {
		...
		uiNamespace setVariable ["AQ_store_display", _display];
		...
		(_display displayCtrl IDC_TITLE) ctrlSetText (_trader getVariable ["AQ_store_name", localize "STR_AQ_STORE_ACTION"]);

		["onInit"] call AQ_store_fnc_main;
	};

	case ("onUnload"): {
		closeDialog 2;
		uiNamespace setVariable ["AQ_store_display", displayNull];

		player setVariable ["AQ_store_trader_current", nil, true];
		AQ_store_item_price = nil;
		AQ_store_category_items = nil;
		AQ_store_vehicles = nil;
		AQ_store_blur ppEffectAdjust [0];
		AQ_store_blur ppEffectCommit 0.3;
	};

	case ("onInit"): {
		if (isNil "AQ_store_blur") then {
			AQ_store_blur = ppEffectCreate ["DynamicBlur", 999];
			AQ_store_blur ppEffectEnable true;
		};

		AQ_store_blur ppEffectAdjust [8];
		AQ_store_blur ppEffectCommit 0.2;
		
		if (isNil "AQ_store_fnc_formatMoney") then {
			AQ_store_fnc_formatMoney = {format ["%1 %2", _this, AQ_store_currencySymbol]};
		};

		// Get all nearby vehicles that we can sell to
		AQ_store_vehicles = ([_trader] call AQ_store_getNearbyVehicles) apply {
		...
		AQ_store_item_price = [];
		AQ_store_category_items = [];
		
		private _items = [];
		private _categories = getArray (missionConfigFile >> "cfgAQAddons" >> "cfgAQStore" >> "stores" >> _trader getVariable ["AQ_store_trader_type", ""] >> "categories");
		{
			private _categoryItems = "true" configClasses (missionConfigFile >> "cfgAQAddons" >> "cfgAQStore" >> "categories" >> _x) apply {[configName _x, getNumber (_x >> "price") max 0]};
			
			[AQ_store_category_items, _x, _categoryItems apply {_x select 0}] call AQ_store_fnc_hashSet;
			{[AQ_store_item_price, _x select 0, _x select 1] call AQ_store_fnc_hashSet} forEach _categoryItems;
				
			_items append _categoryItems;
		} forEach _categories;

		[AQ_store_category_items, "all", _items apply {_x select 0}] call AQ_store_fnc_hashSet;

		["listbox", ["init", []]] call AQ_store_fnc_main;
		["combobox", ["init", []]] call AQ_store_fnc_main;
		["edit", ["init", []]] call AQ_store_fnc_main;
		["text", ["init", []]] call AQ_store_fnc_main;
		["combobox", ["update", []]] call AQ_store_fnc_main;
		
		call AQ_store_fnc_eachFrame;
	};
	
	case ("update"): {
		["text", ["update", ["funds", []]]] call AQ_store_fnc_main;
		["listbox", ["update", []]] call AQ_store_fnc_main;
		["combobox", ["update", []]] call AQ_store_fnc_main;
	};
		...
		switch (toLower _mode) do {
			...
			case ("init"): {
				...
				_ctrlList ctrlAddEventHandler ["LBSelChanged", {
					...
					["text", ["update", ["buy", [_value, _amt]]]] call AQ_store_fnc_main;
					["progressStats", [_data]] call AQ_store_fnc_main; // this cause _value to become 0
					["text", ["update", ["item", []]]] call AQ_store_fnc_main;
					["progressLoad", [CTRLT(IDC_BUY_ITEM_COMBO) getVariable "container", _data, _amt]] call AQ_store_fnc_main;
					["edit", ["update", []]] call AQ_store_fnc_main;
					["button", ["enabled", []]] call AQ_store_fnc_main;
				}];
			};
			...
			case ("update"): {
				...
				private _items = [AQ_store_category_items, _category, []] call AQ_store_fnc_hashGetOrDefault;
				...
				if (_sell) then {
					...
					_sellableItems = [_container] call AQ_store_fnc_getContainerItems;
					
					// Hacky af but basically allows base weapons
					_items = _sellableItems select {
						_x in _items || (_x call AQ_store_fnc_getParentClassname) in _items;
					};
					...
					if (cbChecked _checkCompatible) then { // Compatible items only (wont run if sale checkbox is checked)
						_filterItems = [];
						{_filterItems append (_x call AQ_store_fnc_getCompatibleItems)} forEach [primaryWeapon player, handgunWeapon player, secondaryWeapon player];
				...
				private _money = [player] call AQ_money_fnc_getFunds;
				private _sellFactor = AQ_store_sellFactor min 1 max 0;
				{
					_classname = _x;
					_price = [AQ_store_item_price, _x, 0] call AQ_store_fnc_hashGetOrDefault;
					_priceAdjusted = _price;
					_stock = 0;

//#AQ				// >>> AQ: масштаб цен для UI
						private _k = missionNamespace getVariable ["AQ_PRICE_SCALE", 1];
						_price = round (_price * _k);
						_priceAdjusted = round (_priceAdjusted * _k);
					// <<<
					
					if (_sell) then {
						_parent = _classname call AQ_store_fnc_getParentClassname;		
						_stock = {_x isEqualTo _className || _x isEqualTo _parent} count _sellableItems;
				
						if (_price isEqualTo 0) then {_price = [AQ_store_item_price, _parent, 0] call AQ_store_fnc_hashGetOrDefault};
						_priceAdjusted = _price * _sellFactor;
					} else {
						_stock = [_trader, _classname] call AQ_store_fnc_getTraderStock;
					};

					if (!(_showAvaliable && {_price > _money || _stock < 1})) then {
						_cfg = _classname call AQ_fnc_getConfigClass;
						...
						_ctrlList lbSetTextRight [_idx, _priceAdjusted call AQ_store_fnc_formatMoney];
	...
	case ("combobox"): {
		...
		switch (toLower _mode) do {
			...
			case ("init"): {
				...
				private _cfg = missionConfigFile >> "cfgAQAddons" >> "cfgAQStore";
				private _categories = getArray (_cfg >> "stores" >> _trader getVariable ["AQ_store_trader_type", ""] >> "categories");
				_categories = [_categories, {getText (_cfg >> "categories" >> _x >> "displayName")}, true] call AQ_fnc_sortArray;
				...
				_ctrlPurchase ctrlAddEventHandler ["LBSelChanged", {
					...
					if (cbChecked CTRL(IDC_CHECKBOX+3)) then {
						...
						["progressLoad", [_data, CTRL(IDC_LISTBOX) getVariable "data", CTRLT(IDC_EDIT) getVariable "amt"]] call AQ_store_fnc_main;
						["button", ["enabled", []]] call AQ_store_fnc_main;
					};

					["text", ["update", ["cargo", [_data]]]] call AQ_store_fnc_main;
				}];
			};
			...
			case ("update"): {
				...
				_containers append AQ_store_vehicles;
				{
		...
		switch (toLower _mode) do {
			...
			case ("init"): {
				...
				_ctrlEdit ctrlAddEventHandler ["KeyUp", {
					...
					["progressLoad", [CTRLT(IDC_BUY_ITEM_COMBO) getVariable "container", _listbox getVariable "data", _amt]] call AQ_store_fnc_main;
					["text", ["update", ["buy", [_listbox getVariable "value", _amt]]]] call AQ_store_fnc_main;
					["button", ["enabled", []]] call AQ_store_fnc_main;
				}];
	...
	case ("button"): {
		...
		switch (toLower _mode) do {
			...
			case ("change"): {
				params ["", "_checked"];
				CTRLT(IDC_BUTTON_BUY) ctrlSetText localize (["STR_AQ_STORE_BUTTON_PURCHASE", "STR_AQ_STORE_BUTTON_SELL"] select _checked);
			};
			...
			case ("enabled"): {
				...
				_sale = (1 - (_trader getVariable ["AQ_store_trader_sale", 0])) min 1 max 0;
				_money = [player] call AQ_money_fnc_getFunds;
//#AQ
				private _k = missionNamespace getVariable ["AQ_PRICE_SCALE", 1];
				if ((_price * _k) * _amount * _sale > _money) exitWith {
					_ctrlButton ctrlEnable false;
				};
				...
				_canEquipPlayer = [player, _classname] call AQ_store_fnc_canEquipItem;
			...
			case ("pressed"): {
				...
				if (cbChecked CTRL(IDC_CHECKBOX+3)) then {
					_data remoteExecCall ["AQ_store_fnc_sell", 2, false];
				} else {
					_data pushBack cbChecked CTRLT(IDC_CHECKBOX_BUY);
					_data remoteExecCall ["AQ_store_fnc_purchase", 2, false];
				};
			};
		};
	};
	...
	case ("text"): {
		...
		switch (toLower _mode) do {
			...
			case ("init"): {
				["text", ["update", ["buy", [CTRL(IDC_LISTBOX) getVariable "value", CTRLT(IDC_EDIT) getVariable "amt"]]]] call AQ_store_fnc_main;
				["text", ["update", ["cargo", []]]] call AQ_store_fnc_main;
				["text", ["update", ["funds", []]]] call AQ_store_fnc_main;
				["text", ["update", ["item", []]]] call AQ_store_fnc_main;
			};
			...
			case ("update"): {
				...
				switch (toLower _mode) do {
					...
					case ("buy"): {
						...
						private _money = [player] call AQ_money_fnc_getFunds;
						private _sale = (_trader getVariable ["AQ_store_trader_sale", 0]) min 1 max 0;
						private _sell = cbChecked CTRL(IDC_CHECKBOX + 3);
						_price = _price * ([1, AQ_store_sellFactor min 1 max 0] select _sell);
//#AQ					
						private _k = missionNamespace getVariable ["AQ_PRICE_SCALE", 1];
						_price = round (_price * _k);
						...
						_ctrlText ctrlSetStructuredText parseText format [
							...
							format ["<t color='#aaffaa'>%1</t>", _price call AQ_store_fnc_formatMoney],
							[format ["- %1%2<br/>", _sale * 100, "%"], ""] select (_sale isEqualTo 0),
							format ["<t size='1.1' color='%2'>%3 %1</t>", (_total call AQ_fnc_numberToString) call AQ_store_fnc_formatMoney, ['#b2ec00', '#ea0000'] select (!_sell && {_total > _money}), ["-", "+"] select _sell]
						];
					...
					case ("funds"): {
						private _money = ([player] call AQ_money_fnc_getFunds) call AQ_fnc_numberToString;
						((uiNamespace getVariable ["AQ_store_display", displayNull]) displayCtrl IDC_FUNDS) ctrlSetStructuredText parseText (_money call AQ_store_fnc_formatMoney);
					};
					...
					case ("item"): {
						...
						_config = _classname call AQ_fnc_getConfigClass;
						_ctrlText ctrlSetStructuredText parseText ([
							getText (missionConfigFile >> "cfgAQAddons" >> "cfgAQStore" >> "categories" >>  CTRL(IDC_COMBO_CATEGORY) getVariable "data" >> _classname >> "description"),
							[_config >> "Library" >> "libTextDesc", ""] call AQ_fnc_getConfigValue,
							[_config >> "descriptionShort", ""] call AQ_fnc_getConfigValue
						...
						_stockText = format [
							...
							[[localize "STR_AQ_STORE_TEXT_NOSTOCK", localize "STR_AQ_STORE_TEXT_INSTOCK"] select (_stock > 0), "AVAILABLE"] select _sell,
							["", ":  " + (_stock call AQ_fnc_numberToString)] select (_stock > 0)
						];
						_price = (_ctrlList lbValue _idx) * ([1, AQ_store_sellFactor min 1 max 0] select _sell);
						...
						_ctrlTitle ctrlSetStructuredText parseText format [
							...
							_ctrlList lbText _idx, (_price call AQ_fnc_numberToString) call AQ_store_fnc_formatMoney, toUpper localize "STR_AQ_STORE_TEXT_PRICE", _stockText
						];
	...
	case "progressLoad": {
		...
		_currentLoad = [_container] call AQ_store_fnc_getCargoMass;
		...
		_load = _classname call AQ_store_fnc_getItemMass;
		if ([_classname] call AQ_store_fnc_getItemType isEqualTo 3) then {
			_arrayCargo = [];
			...
			{_load = _load + ((_x select 0) call AQ_store_fnc_getItemMass) * (_x select 1)} forEach _arrayCargo;
		};
	...
	case "progressStats": {
		...
		private _cfg = (_data param [0, ""]) call AQ_fnc_getConfigClass;
		private _stats = ([_cfg] call AQ_store_fnc_getItemStats);
