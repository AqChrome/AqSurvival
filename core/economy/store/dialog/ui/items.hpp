...
class ItemCheckboxSort: RscItemCheckboxGreen {
	...
	w = GRID_X(BAR_W);
	h = GRID_Y(BAR_H);
	onCheckedChanged = "['listbox', ['sort', _this]] call AQ_store_fnc_main;";
	sizeEx = FONT(1);
	color[] = {1, 1, 1, 0.7};
	colorActive[] = {1, 1, 1, 1};
	...
	shadow = 2;
	tooltip = "$STR_AQ_STORE_CHECKBOX_SORT"; //TODO tooltip
...
class ItemCheckboxAvailable: RscItemCheckboxGreen {
	...
	w = GRID_X(BAR_W);
	h = GRID_Y(BAR_H);
	onCheckedChanged = "['listbox', ['update', []]] call AQ_store_fnc_main;";
	tooltip = "$STR_AQ_STORE_CHECKBOX_AVAILABLE";
};
...
class ItemCheckboxCompatible: RscItemCheckboxGreen {
	...
	h = GRID_Y(BAR_H);
	onCheckedChanged = "['listbox', ['update', []]] call AQ_store_fnc_main;";
	tooltip = "$STR_AQ_STORE_CHECKBOX_COMPATIBLE";
};
...
class ItemCheckboxSell: RscItemCheckboxGreen {
	...
	w = GRID_X(BAR_W);
	h = GRID_Y(BAR_H);
	onCheckedChanged = "['button', ['change', _this]] call AQ_store_fnc_main; ['listbox', ['update', []]] call AQ_store_fnc_main;";
	tooltip = "$STR_AQ_STORE_CHECKBOX_SELLFILTER";
};
