...
class RscDisplayStore {
	...
	onLoad = "['onLoad', _this] call AQ_store_fnc_main";
	onUnload = "['onUnload', _this] call AQ_store_fnc_main";
	...
	class controls {
		class CloseButton: RscItemButtonClose {
			...
			action = "['onUnload'] call AQ_store_fnc_main;";
			colorText[] = {1, 1, 1, 0.7};
			color[] = {1, 1, 1, 0.7};
		};
		...
		*/
		...
		class HelpButton {
			...
			url = "https://github.com/HallyG/AQ_Store/wiki/Usage";
		...
		class AQ_store_dialog: RscItemCtrlGroupNoScrollbars {
			idc = IDC_GROUP_ITEMS;
			x = DIALOG_X;
			y = DIALOG_Y + GRID_Y(BAR_H + SPACE_H + SPACE_H2);
			w = GRID_X(BUFFER_W + LIST_W + 2);
			h = GRID_Y(STORE_H);
