...
private _gw = 0.88 * _sw; private _gh = 0.84 * _sh;
...
_bg ctrlSetText "aq_tablet/ui/tasks_bg_ca.paa";
...
private _html = "<t align='left' size='1.0' color='#2a2a2a'>";
{
  ...
  _html = _html + format ["<t size='1.25' font='PuristaSemiBold'>%1</t><br/>%2<br/>", _title, _body];
  if (_img != "") then { _html = _html + format ["<img image='%1' align='left' w='380' h='380'/><br/>", _img]; };
  _html = _html + "<br/>";
} forEach _tasks;
_html = _html + "</t>";
...
// Крестик «×» (закрыть планшет полностью)
private _btnX = _disp ctrlCreate ["RscButton", -1, _grp];
_btnX ctrlSetText "×";
_btnX ctrlSetPosition [_gw - 0.08*_gw, 0.02*_gh, 0.06*_gw, 0.05*_gh];
_btnX ctrlCommit 0;
_btnX ctrlAddEventHandler ["ButtonClick", { [] call AQTB_fnc_close }];
